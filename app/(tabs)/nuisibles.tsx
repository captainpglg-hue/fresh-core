...
import { ArrowLeft, CheckCircle, Circle, AlertTriangle, Calendar, Trash2 } from 'lucide-react-native';
import { differenceInDays, format } from 'date-fns';
import { fr } from 'date-fns/locale';

const PEST_TYPES = ['Rongeur', 'Insecte rampant', 'Insecte volant', 'Oiseau', 'Autre'];

export default function NuisiblesScreen() {
  const router = useRouter();
  const { establishment } = useAuthStore();
  const { checkpoints, controls, loadData, validateCheckpoint, reportAnomaly, addIntervention, addCheckpoint, removeCheckpoint, getNextVisitDate } = usePestStore();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Nuisibles</Text>
